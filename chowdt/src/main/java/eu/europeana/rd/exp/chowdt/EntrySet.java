...
{
    public void newEntry(String cho, String wdt, String anno)
    {
        if ( add(new Entry(cho, wdt, anno)) ) { return; }
    ...
    public EntrySet loadFromCVS(File file)
    {
        ...
        try {
            parser = CSVParser.parse(file, Charset.forName("UTF-8")
                                   , CSVFormat.EXCEL);
            for ( CSVRecord r : parser ) { loadEntry(r); }
        }
    ...
    private void loadEntry(CSVRecord r)
    {
        int size = r.size();
        newEntry(r.get(0), r.get(1), (size > 2 ? r.get(2) : null));
    }
    ...
    static class Entry implements Comparable<Entry>
    {
        ...
        public Entry(String cho, String wdt, String anno)
        {
            this.cho  = cho;
            this.wdt  = wdt;
            this.anno = anno;
        ...
        public Entry(String cho, String wdt) { this ( cho, wdt, null); }
