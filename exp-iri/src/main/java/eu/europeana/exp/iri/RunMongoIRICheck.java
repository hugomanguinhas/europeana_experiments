...
{
    private static Logger _log = Logger.getLogger(RunMongoIRICheck.class);
    ...
    public RunMongoIRICheck(MongoDatabase db) { _db = db; }
        ...
        try {
            ...
            try {
                while (iter.hasNext())
                {
                    cursor++;
                    if ( cursor % 1000 == 0 ) {
                        _log.info("Validated [" + cursor + "] of [" + size + "]");
                    }
    ...
    {
        _log.error("Exception validating IRI <" + str + ">", e);
    }
}
